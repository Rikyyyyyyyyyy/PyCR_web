...
class PyCRThread(threading.Thread):
    ...
    def run(self):
        # try:
            ...
            os.remove(zipName)
            if self.isSentEmail:
                sent_email.runSendEmail(self.cur_user.email,'featureSelection/temp/zipOutput/output'+str(self.task.pk)+'.zip',self.base_dir,self.cur_user.username,self.task.task_name, self.task.isExternal, self.task.rankingAlgorithm, self.task.rocType, self.task.tupaType, self.task.scaleType,self.task.iterations,self.task.survivalRate)
